 ...
 *
 * Plugin bundles are built with `external: ["@sprintai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__sprintPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__sprintPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__sprintPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__sprintPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@sprintai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@sprintai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@sprintai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@sprintai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@sprintai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__sprintPluginBridge__) {
 ...
 * self-register via `window.sprintPlugins.registerReactComponent()`.
 ...
 * of `@sprintai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
