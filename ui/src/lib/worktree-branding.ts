...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("sprint-worktree-enabled") !== "true") return null;

  const name = readMetaContent("sprint-worktree-name");
  const color = normalizeHexColor(readMetaContent("sprint-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("sprint-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
