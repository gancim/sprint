...
import { buildAgentMentionHref, buildProjectMentionHref } from "@sprintai/shared";
...
describe("MarkdownBody", () => {
  ...
  it("renders agent and project mentions as chips", () => {
    const html = renderToStaticMarkup(
      <ThemeProvider>
        <MarkdownBody>
          {`[@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Sprint App](${buildProjectMentionHref("project-456", "#336699")})`}
        </MarkdownBody>
      </ThemeProvider>,
    );
    ...
    expect(html).toContain("--sprint-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--sprint-mention-project-color:#336699");
  });
});
