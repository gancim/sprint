...
type SprintSprite = (typeof SPRINT_SPRITES)[number];
...
function spriteSize(sprite: SprintSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SPRINT_SPRITES[(baseCol + baseRow) % SPRINT_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SPRINT_SPRITES[Math.floor(Math.random() * SPRINT_SPRITES.length)]!;
