...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Sprint API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "sprintApiUrl",
                  String(config.sprintApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "sprintApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://sprint.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "sprint"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Sprint persists a device key during onboarding so pairing approvals
