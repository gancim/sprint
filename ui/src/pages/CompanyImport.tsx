...
import type { CreateConfigValues } from "@sprintai/adapter-utils";
import {
  type FileTreeNode,
...
export function CompanyImport() {
  ...
  const ceoAdapterType = useMemo(() => {
    ...
    const ceo = companyAgents.find((a) => a.role === "scrum_master");
    return ceo?.adapterType ?? "claude_local";
  }, [companyAgents]);

  const localZipHelpText =
    "Upload a .zip exported directly from Sprint. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.";
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Choose a GitHub repo or upload a local Sprint zip package.
          </p>
        </div>
