...
/**
 * Filter .sprint.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterSprintYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://sprint.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm sprintai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Sprint](https://sprint.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Sprint](https://sprint.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .sprint.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .sprint.yaml
    const yamlPath = exportData.sprintExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterSprintYaml(exportData.files[yamlPath], checkedFiles);
    }
