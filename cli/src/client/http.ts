...
export class SprintApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-sprint-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Sprint API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Sprint server is not running, the configured URL is wrong, or the request is being blocked before it reaches Sprint.",
    "",
    "Try:",
    "- Start Sprint with `pnpm dev` or `pnpm sprintai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Sprint is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`SPRINT_API_URL\`.`,
  );
  return lines.join("\n");
}
