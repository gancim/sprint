...
import { ApiConnectionError, ApiRequestError, SprintApiClient } from "../client/http.js";
...
describe("SprintApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new SprintApiClient({
    ...
    expect(headers["x-sprint-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new SprintApiClient({ apiBase: "http://localhost:3100" });
