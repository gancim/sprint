...
const JWT_SECRET_ENV_KEY = "SPRINT_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Sprint environment variables",
    "# Generated by Sprint CLI commands",
...
export function resolveSprintEnvFile(configPath?: string): string {
...
export function loadSprintEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeSprintEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readSprintEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeSprintEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeSprintEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readSprintEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeSprintEnvEntries(next, filePath);
  return next;
}
