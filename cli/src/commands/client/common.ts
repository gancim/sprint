...
import { ApiRequestError, SprintApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Sprint config file")
    .option("-d, --data-dir <path>", "Sprint data directory root (isolates state from ~/.sprint)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Sprint API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.SPRINT_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.SPRINT_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.SPRINT_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set SPRINT_COMPANY_ID, or set context profile companyId via `sprintai context set`.",
    );
  }

  const api = new SprintApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.SPRINT_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.SPRINT_SERVER_PORT || "");
