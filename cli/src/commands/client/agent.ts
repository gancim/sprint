...
import type { Agent } from "@sprintai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveSprintSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export SPRINT_API_URL='${escaped(input.apiBase)}'`,
    `export SPRINT_COMPANY_ID='${escaped(input.companyId)}'`,
    `export SPRINT_AGENT_ID='${escaped(input.agentId)}'`,
    `export SPRINT_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Sprint skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Sprint skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveSprintSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Sprint skills directory. Expected ./skills in the repo checkout.",
              );
            }
