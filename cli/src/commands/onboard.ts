...
type OnboardDefaults = Pick<SprintConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "SPRINT_PUBLIC_URL",
  "DATABASE_URL",
  "SPRINT_DB_BACKUP_ENABLED",
  "SPRINT_DB_BACKUP_INTERVAL_MINUTES",
  "SPRINT_DB_BACKUP_RETENTION_DAYS",
  "SPRINT_DB_BACKUP_DIR",
  "SPRINT_DEPLOYMENT_MODE",
  "SPRINT_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "SPRINT_ALLOWED_HOSTNAMES",
  "SPRINT_AUTH_BASE_URL_MODE",
  "SPRINT_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "SPRINT_STORAGE_PROVIDER",
  "SPRINT_STORAGE_LOCAL_DIR",
  "SPRINT_STORAGE_S3_BUCKET",
  "SPRINT_STORAGE_S3_REGION",
  "SPRINT_STORAGE_S3_ENDPOINT",
  "SPRINT_STORAGE_S3_PREFIX",
  "SPRINT_STORAGE_S3_FORCE_PATH_STYLE",
  "SPRINT_SECRETS_PROVIDER",
  "SPRINT_SECRETS_STRICT_MODE",
  "SPRINT_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveSprintInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.SPRINT_PUBLIC_URL?.trim() ||
    process.env.SPRINT_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.SPRINT_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.SPRINT_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.SPRINT_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.SPRINT_ALLOWED_HOSTNAMES
    ? process.env.SPRINT_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.SPRINT_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.SPRINT_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.SPRINT_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.SPRINT_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.SPRINT_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.SPRINT_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.SPRINT_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.SPRINT_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.SPRINT_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.SPRINT_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.SPRINT_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.SPRINT_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.SPRINT_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.SPRINT_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.SPRINT_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "SPRINT_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<SprintConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printSprintCliBanner();
  p.intro(pc.bgCyan(pc.black(" sprintai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveSprintInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: SprintConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@sprintai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `sprintai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("SPRINT_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.SPRINT_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("SPRINT_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("SPRINT_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }
  ...
  p.note(
    [
      ...
      "Agent auth: SPRINT_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("sprintai run")}`,
      `Reconfigure later: ${pc.cyan("sprintai configure")}`,
      `Diagnose setup: ${pc.cyan("sprintai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.SPRINT_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("sprintai run")}`,
        `Then: ${pc.cyan("sprintai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
