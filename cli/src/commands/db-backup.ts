...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@sprintai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://sprint:sprint@127.0.0.1:${port}/sprint`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" sprint db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveSprintInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "sprint";
