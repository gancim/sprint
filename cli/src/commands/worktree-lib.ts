...
export const DEFAULT_WORKTREE_HOME = "~/.sprint-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".sprint");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: SprintConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "sprint",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    SPRINT_HOME: paths.homeDir,
    SPRINT_INSTANCE_ID: paths.instanceId,
    SPRINT_CONFIG: paths.configPath,
    SPRINT_CONTEXT: paths.contextPath,
    SPRINT_IN_WORKTREE: "true",
    ...(branding?.name ? { SPRINT_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { SPRINT_WORKTREE_COLOR: branding.color } : {}),
  };
}
