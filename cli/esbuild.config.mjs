/**
 * esbuild configuration for building the sprintai CLI for npm.
 *
 * Bundles all workspace packages (@sprintai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @sprintai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@sprintai/")) {
      externals.add(name);
    }
  }
