...
└── .sprint.yaml            (Sprint vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .sprint.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [Sprint](https://github.com/sprintai/sprint)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to Sprint (https://github.com/sprintai/sprint)
- A "Getting Started" section explaining how to import: `sprintai company import --from <path>`
...
## .sprint.yaml Guidelines

The `.sprint.yaml` file is the Sprint vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — Sprint will use its default. Specifying an unknown adapter type causes an import error.

Sprint's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.sprint.yaml`.
