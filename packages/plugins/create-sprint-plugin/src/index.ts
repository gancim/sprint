...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".sprint-sdk");
...
/**
 * Generate a complete Sprint plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Sprint plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "sprint-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["sprint", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@sprintai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@sprintai/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { SprintPluginManifestV1 } from "@sprintai/plugin-sdk";

const manifest: SprintPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@sprintai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@sprintai/plugin-sdk\` and \`@sprintai/shared\` from a local Sprint checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.sprint-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-sprint-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <sprint-sdk-path>]");
    process.exit(1);
  }
