/**
 * Core types for the Sprint plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@sprintai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @sprintai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Sprint object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Sprint UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Sprint domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Sprint domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Sprint secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Sprint secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@sprintai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@sprintai/plugin-sdk";
 *
