...
Official TypeScript SDK for Sprint plugin authors.

- **Worker SDK:** `@sprintai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@sprintai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@sprintai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@sprintai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@sprintai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@sprintai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@sprintai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@sprintai/plugin-sdk/ui/hooks` | Hooks only |
| `@sprintai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@sprintai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@sprintai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@sprintai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@sprintai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@sprintai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Sprint app. They can call ordinary Sprint HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Sprint instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@sprintai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@sprintai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Sprint information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@sprintai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@sprintai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@sprintai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@sprintai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@sprintai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@sprintai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@sprintai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@sprintai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@sprintai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@sprintai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@sprintai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@sprintai/plugin-sdk/bundlers";
...
```bash
sprint-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@sprintai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__sprint__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__sprint__/events` streams `reload` SSE events on UI build changes
