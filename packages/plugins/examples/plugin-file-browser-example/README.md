...
Example Sprint plugin that demonstrates:
...
```bash
pnpm --filter @sprintai/plugin-file-browser-example build
pnpm sprintai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm sprintai plugin uninstall sprint-file-browser-example --force
```
...
- Optional: use `sprint-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
