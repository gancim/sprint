...
const SPRINT_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface SprintSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: SprintSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "sprint_required",
      originLabel: "Required by Sprint",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Sprint",
    readOnly: false,
  };
}
...
export function buildSprintEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.SPRINT_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.SPRINT_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.SPRINT_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.SPRINT_API_URL = apiUrl;
  return vars;
}
...
export async function resolveSprintSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...SPRINT_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listSprintSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<SprintSkillEntry[]> {
  const root = await resolveSprintSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `sprintai/sprint/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Sprint skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Sprint cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredSprintRuntimeSkills(value: unknown): SprintSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: SprintSkillEntry[] = [];
...
export async function readSprintRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<SprintSkillEntry[]> {
  const configuredEntries = normalizeConfiguredSprintRuntimeSkills(config.sprintRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listSprintSkillEntries(moduleDir, additionalCandidates);
}

export async function readSprintSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listSprintSkillEntries(moduleDir);
...
export function readSprintSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.sprintSkillSync;
...
function canonicalizeDesiredSprintSkillReference(
  reference: string,
...
export function resolveSprintDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readSprintSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredSprintSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeSprintSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.sprintSkillSync;
  const current =
  ...
  next.sprintSkillSync = current;
  return next;
}

export async function ensureSprintSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Sprint server itself is started from
    // within a Claude Code session (e.g. `npx sprintai run` in a terminal
