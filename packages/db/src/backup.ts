...
function resolveSprintInstanceId(): string {
  const raw = process.env.SPRINT_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveSprintHomeDir(), "instances", resolveSprintInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveSprintHomeDir(), "instances", resolveSprintInstanceId(), "data", "backups");
}
