...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://sprint:sprint@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "sprint");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://sprint:sprint@127.0.0.1:${preferredPort}/sprint`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://sprint:sprint@127.0.0.1:${port}/sprint`,
  ...
  const adminConnectionString = `postgres://sprint:sprint@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "sprint");

  return {
    connectionString: `postgres://sprint:sprint@127.0.0.1:${selectedPort}/sprint`,
