...
Use when:
- You want Sprint to run Cursor Agent CLI locally as the agent runtime
...
- Sprint auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$sprint" and related skills on local runs.
- Sprint auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
