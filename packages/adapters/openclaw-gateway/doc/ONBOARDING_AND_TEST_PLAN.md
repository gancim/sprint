...
## Scope
This plan is now **gateway-only**. Sprint supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Sprint skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Sprint task.
...
```bash
pnpm --filter @sprintai/server test:run -- openclaw-gateway
pnpm --filter @sprintai/server typecheck
pnpm --filter @sprintai/ui typecheck
pnpm --filter sprintai typecheck
```
