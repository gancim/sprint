...
Use when:
- You want Sprint to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Sprint server.
...
- workspaceRuntime (object, optional): desired runtime service intents; Sprint forwards these in a standardized sprint.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- sprintApiUrl (string, optional): absolute Sprint base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default sprint)

Standard outbound payload additions:
- sprint (object): standardized Sprint context added to every gateway agent request
- sprint.workspace (object, optional): resolved execution workspace for this run
- sprint.workspaces (array, optional): additional workspace hints Sprint exposed to the run
- sprint.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
