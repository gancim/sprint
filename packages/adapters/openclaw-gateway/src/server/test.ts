...
import { asString, parseObject } from "@sprintai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Sprint server host.",
        });
      }
    } catch (err) {
      checks.push({
