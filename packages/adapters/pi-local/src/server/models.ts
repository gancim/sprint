...
import type { AdapterModel } from "@sprintai/adapter-utils";
import { asString, runChildProcess } from "@sprintai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.SPRINT_PI_COMMAND === "string" &&
    process.env.SPRINT_PI_COMMAND.trim().length > 0
      ? process.env.SPRINT_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["SPRINT_", "npm_", "NPM_"] as const;
