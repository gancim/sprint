---
name: sprint-create-plugin
description: >
  Create new Sprint plugins with the current alpha SDK/runtime. Use when
...
# Create a Sprint Plugin

Use this skill when the task is to create, scaffold, or document a Sprint plugin.
...
node packages/plugins/create-sprint-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Sprint repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.sprint-sdk/`:

```bash
pnpm --filter @sprintai/create-sprint-plugin build
node packages/plugins/create-sprint-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Sprint from an absolute local path during development
