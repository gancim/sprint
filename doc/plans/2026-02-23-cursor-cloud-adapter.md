...
This document defines the V1 design for a Sprint adapter that integrates with
...
4. synthesized stdout events for Sprint UI/CLI
...
3. **Agent callback auth to Sprint** uses a bootstrap exchange flow (no long-lived Sprint key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Sprint endpoints, not full SKILL.md prompt injection.
...
- `sprintPublicUrl` (optional override; else `SPRINT_PUBLIC_URL` env)
...
## Sprint Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `SPRINT_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.sprintPublicUrl`
2. `process.env.SPRINT_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Sprint credentials in prompt text.

Flow:

1. Before launch/follow-up, Sprint mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Sprint validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Sprint API calls.
...
1. Prompt includes a compact instruction to fetch skills from Sprint.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/sprint`
   - `GET /api/skills/sprint-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <sprintPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Sprint run context.
...
5. `sprintPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Sprint skill endpoints |
| Sprint auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Sprint context.
3. Add richer PR metadata surfacing in Sprint UI.
