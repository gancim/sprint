...
Add a single storage subsystem for Sprint that supports:
...
- API serves attachment bytes through authenticated Sprint endpoints.
...
- Default local path is under instance root: `~/.sprint/instances/<instanceId>/data/storage`.
...
- `sprintai onboard` writes a valid `storage` config block by default.
- `sprintai configure --section storage` can switch between local and s3 modes.
...
- `sprintai doctor` reports actionable storage status.
