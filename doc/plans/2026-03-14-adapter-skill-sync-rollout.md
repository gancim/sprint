...
This document defines the rollout plan for adapter-wide skill support in Sprint.
...
- Sprint stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Sprint currently has these adapters:
...
1. Desired skills live in Sprint for every adapter.
...
These adapters have a stable local skills directory that Sprint can read and manage.
...
These adapters do not have a meaningful Sprint-owned persistent install state.
...
- desired skills stored in Sprint
...
- runtime already injects Sprint skills into `~/.cursor/skills`
...
- runtime already injects Sprint skills into `~/.gemini/skills`
...
- runtime already injects Sprint skills into `~/.pi/agent/skills`
...
- Sprint can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Sprint skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Sprint may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Sprint skills that are clearly marked as Sprint-managed
...
- Sprint stores desired skills only
...
- avoid destructive wording unless Sprint can prove a skill is Sprint-managed
...
That gets Sprint from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
