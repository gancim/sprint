...
Today release smoke testing for published Sprint packages is manual and shell-driven:
...
- runs `sprintai@${SPRINTAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@sprint.local`
- password: `sprint-smoke-password`
...
- the suite passes locally against both:
  - `SPRINTAI_VERSION=canary`
  - `SPRINTAI_VERSION=latest`
...
1. A published `sprintai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `sprintai@latest` Docker install can be smoke-tested by Playwright in CI.
