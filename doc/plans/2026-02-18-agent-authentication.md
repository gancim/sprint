...
Agents need API keys to authenticate with Sprint. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Sprint
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Sprint generates a token and passes it directly to the agent
...
`SPRINT_API_URL`, `SPRINT_AGENT_ID`, `SPRINT_COMPANY_ID`. We need to
add a `SPRINT_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Sprint CLI
...
```
sprint auth login
# Opens browser -> user confirms -> token stored at ~/.sprint/credentials
```
...
managed by the Sprint server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Sprint.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Sprint SKILL.md (or a link to it)
   - What information Sprint needs from the agent (e.g. webhook URL, adapter
...
5. Sprint stores the pending registration.
...
7. On approval, Sprint provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Sprint issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.sprint.ing/skills/sprint/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Sprint should send you heartbeats",
...
1. Generate a Sprint invite link for the company.
...
4. A Sprint company member approves the new agent.
5. Sprint begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `SPRINT_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `sprintai auth login` for developer-managed remote agents.                                      |
