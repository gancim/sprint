...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Sprint plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Sprint.
...
- package: `@sprintai/plugin-kitchen-sink-example`
- plugin id: `sprint.kitchen-sink-example` or `sprint-kitchen-sink-example`

Recommendation: use `sprint-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Sprint event such as `issue.created`
...
Recommendation: default to a Sprint-local endpoint or a stable public echo endpoint to avoid flaky docs.
