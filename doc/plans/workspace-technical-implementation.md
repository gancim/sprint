...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/sprint-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `SPRINT_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `SPRINT_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Sprint may create host-local worktrees, processes, and paths
- `cloud`
  - Sprint should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `sprint | github | vercel | s3 | custom`
...
- use `SPRINT_EXECUTION_TOPOLOGY` as a defaulting guardrail
