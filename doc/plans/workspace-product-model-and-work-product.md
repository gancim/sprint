...
Sprint needs to support two very different but equally valid ways of working:
...
Today, Sprint already has the beginnings of this model:
...
- making Sprint feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Sprint users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Sprint deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Sprint into a full code review product
...
This object must be recorded explicitly so that Sprint can:
...
Sprint should treat PRs as a type of work product linked back to:
...
Sprint already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Sprint must be a first-class deployment mode

Sprint cannot assume that it is running on the same machine as the code.

In cloud deployments, Sprint may:
...
- `Runtime service`: a process or service Sprint owns or tracks for a workspace
...
It also lets cloud-hosted Sprint deployments point at codebases and remotes without pretending the Sprint host has direct filesystem access.
...
Without an explicit `execution workspace` record, Sprint has nowhere stable to attach:
...
Sprint needs a single place to show:
...
- Sprint-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Sprint owning the execution host
...
## 1. Sprint host and execution host must be decoupled
...
without the Sprint host itself running local git or local preview processes.
...
Sprint should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Sprint.
...
Sprint should keep workspace automation centered on a small set of hooks:
...
Sprint should provide the equivalent visibility in the project `Code` surface:
...
Sprint should treat execution workspaces as the natural home for:
...
Sprint should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Sprint should continue treating cleanup policy as part of the core workspace model:
...
Sprint should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Sprint host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Sprint should borrow Worktrunk's good execution patterns while keeping the broader Sprint model:
...
That keeps Sprint focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Sprint deployments remain first-class

That is a better fit for Sprint than either extreme:
