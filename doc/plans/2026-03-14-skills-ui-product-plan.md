...
This document defines the product and UI plan for skill management in Sprint.
...
- Sprint company import/export can include skills as package content
...
3. The UI must always tell the truth about what Sprint knows:
   - desired state in Sprint
...
Sprint should treat skills at two scopes:
...
- created locally inside Sprint later
...
7. “Understand what Sprint knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Sprint-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Sprint did not install.

These should be clearly marked:

- external
- not managed by Sprint
...
- `desired_only`: Sprint wants it, adapter does not show it yet
- `external`: adapter has it but Sprint does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Sprint only tracks desired state
...
- Sprint can still track desired skills
...
- `.sprint.yaml` may add Sprint-specific fidelity, but should not replace the base shortname association model
...
- `.sprint.yaml` may add Sprint-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Sprint should have a company-scoped skill model or managed package model representing:
...
Sprint should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Sprint-specific.
...
That gives Sprint one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
