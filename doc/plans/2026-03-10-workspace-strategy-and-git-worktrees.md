...
`PAP-447` asks how Sprint should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Sprint instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Sprint:

- some operators use Sprint to manage Sprint and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Sprint
- Claude and Codex expose different built-in affordances, so Sprint should not overfit to one tool

## Core Product Decision

Sprint should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Sprint
...
- Sprint should not assume that every adapter can see or use a host filesystem path directly
...
If Sprint makes worktrees agent-first, it will blur:
...
- Sprint core resolves and records execution workspace state
...
That gives Sprint room to support:
...
- Sprint handles workspace runtime services under the hood using project/workspace policy
...
Once Sprint is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Sprint cannot infer
...
- when issue moves to done, should Sprint auto-commit?
- should it auto-push?
...
Sprint already has the right foundation for a project-first model.
...
For server-spawned local adapters, Sprint already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Sprint should mirror directly

Implication:

- for `codex_local`, Sprint should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Sprint should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Sprint should separate:
...
Sprint should be able to express intentions such as:
...
These are not specific to Sprint. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".sprint/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Sprint-specific `devServer` flag.
...
Sprint should distinguish between:
...
Sprint should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Sprint should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Sprint
...
For runtime services specifically, Sprint should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Sprint-only dev-server flags
...
3. Inject resolved execution workspace details into `context.sprintWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Sprint instance can be reused through env/config injection when appropriate
...
- Sprint can decide whether to reuse or start a fresh service deterministically
...
3. Test against Sprint-on-Sprint workflows first.
...
7. Workspace runtime services are modeled generically, not as Sprint-specific dev-server toggles.
