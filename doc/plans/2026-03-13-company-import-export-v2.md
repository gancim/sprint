...
This document defines the next-stage plan for Sprint company import/export.

The core shift is:

- move from a Sprint-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Sprint.
...
Sprint already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Sprint
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `sprint.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.sprint.yaml` sidecar for high-fidelity Sprint-specific details
...
11. adapter-aware skill sync surfaces so Sprint can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Sprint at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Sprint sidecar.
...
Sprint must not redefine `SKILL.md`.
...
- Sprint-specific fidelity lives in `.sprint.yaml`
- Sprint may resolve and install `SKILL.md` packages, but it must not require a Sprint-only skill format
...
### 5.4 Base Package Vs Sprint Extension
...
- base package:
  ...
  - no Sprint-only runtime fields by default
- Sprint extension:
  - `.sprint.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Sprint tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`sprint.manifest.json` is not part of the future package direction.
...
Sprint import/export should support these entity kinds:
...
- Sprint can treat “this set of agents came from team package X” as the imported-team model
...
Sprint should support source references in package metadata with:
...
Sprint should define a new adapter surface area around skills:
...
Planned Sprint behavior:

- if an adapter supports read, Sprint should show current skills in the UI
- if an adapter supports write, Sprint should let the user enable/disable imported skills
- if an adapter supports sync, Sprint should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.sprint.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.sprint.yaml` alongside the base package
...
## 10. Storage Model Inside Sprint
...
Sprint should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `sprintai company export <company-id> --out <path>`
- `sprintai company import <path-or-url> --dry-run`
- `sprintai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Sprint storage, or only referenced at import time?
...
3. Should Sprint support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Sprint aligned with:
