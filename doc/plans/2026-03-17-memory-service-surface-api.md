# Sprint Memory Service Plan

## Goal

Define a Sprint memory service and surface API that can sit above multiple memory backends, while preserving Sprint's control-plane requirements:

- company scoping
- auditability
- provenance back to Sprint work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current Sprint architecture in:
...
Sprint should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Sprint plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. Sprint owns routing, provenance, and accounting

Providers should not decide how Sprint entities map to governance.

Sprint core should own:
...
The normalized Sprint scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through Sprint.

Sprint should log every operation, whether the provider is local or external.
...
## What Sprint Should Persist

Sprint should not mirror the full provider memory corpus into Postgres unless the provider is a Sprint-managed local provider.

Sprint core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, Sprint may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, Sprint may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, Sprint may capture selected issue comments or issue documents as memory sources.
...
Sprint should give agents both automatic recall and explicit tools, with simple guidance:
...
Sprint needs a first-class UI for memory, otherwise providers become black boxes.
...
This is important because a memory system that "works" but silently burns budget is not acceptable in Sprint.
...
- it matches Sprint's local-first posture
- it is inspectable
...
- Do we want Sprint-managed extraction pipelines at all, or should built-ins be the only place where Sprint owns extraction?
...
- Sprint owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Sprint a stable "memory service" without locking the product to one memory philosophy or one vendor.
