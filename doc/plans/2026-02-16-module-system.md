...
Sprint's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Sprint's API, UI, and data model | Yes |
...
modules/
  observability/
    sprint.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@sprintai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@sprintai/core";
...
```
1. Scan modules/ directory for sprint.module.json manifests
...
```jsonc
// sprint.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@sprintai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@sprintai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:sprint/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.sprint.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm sprintai store list                    # browse available modules and templates
pnpm sprintai store install <module-id>     # install a module
pnpm sprintai store import <template-id>    # import a company template
pnpm sprintai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Sprint instance | `server:started` |
...
4. **Module config** — `sprint.config.json` with per-module config, env var interpolation
...
10. **`@sprintai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
