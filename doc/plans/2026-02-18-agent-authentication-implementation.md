...
- New config/env settings:
  - `SPRINT_AGENT_JWT_SECRET`
  - `SPRINT_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `SPRINT_AGENT_JWT_ISSUER` (default: `sprint`)
  - `SPRINT_AGENT_JWT_AUDIENCE` (default: `sprint-api`)
...
1. In:
   ...
   inject `SPRINT_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `SPRINT_API_KEY`, do not overwrite it.
- Continue injecting:
  - `SPRINT_AGENT_ID`
  - `SPRINT_COMPANY_ID`
  - `SPRINT_API_URL`
...
- Local adapters authenticate without manual `SPRINT_API_KEY` config.
