...
This guide describes the current, implemented way to create a Sprint plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Sprint app.
...
```bash
pnpm --filter @sprintai/create-sprint-plugin build
node packages/plugins/create-sprint-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Sprint repo:

```bash
pnpm --filter @sprintai/create-sprint-plugin build
node packages/plugins/create-sprint-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/sprint/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@sprintai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@sprintai/plugin-sdk` from the local Sprint checkout into a `.sprint-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Sprint from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@sprintai/plugin-sdk/ui`
