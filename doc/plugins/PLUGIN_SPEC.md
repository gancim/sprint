# Sprint Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Sprint's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Sprint app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Sprint HTTP APIs directly.
...
Sprint plugin design is based on the following assumptions:

1. Sprint is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Sprint business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Sprint core.
...
2. Let plugins add major capabilities without editing Sprint core.
...
4. Depend on project-local plugin folders such as `.sprint/plugins`.
...
The single Sprint deployment an operator installs and controls.

### 5.2 Company

A first-class Sprint business object inside the instance.
...
A trusted in-process extension loaded directly by Sprint core.
...
An installable instance-wide extension package loaded through the Sprint plugin runtime.
...
Sprint has two extension classes.
...
Sprint already has a concrete workspace model:
...
Plugins live under the Sprint instance directory.

Suggested layout:

- `~/.sprint/instances/default/plugins/package.json`
- `~/.sprint/instances/default/plugins/node_modules/`
- `~/.sprint/instances/default/plugins/.cache/`
- `~/.sprint/instances/default/data/plugins/<plugin-id>/`
...
Sprint should add CLI commands:

- `pnpm sprintai plugin list`
- `pnpm sprintai plugin install <package[@version]>`
- `pnpm sprintai plugin uninstall <plugin-id>`
- `pnpm sprintai plugin upgrade <plugin-id> [version]`
- `pnpm sprintai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@sprint/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface SprintPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Sprint agents can use during runs.
...
- Sprint server starts one worker process per installed plugin
...
Receives one typed Sprint domain event.
...
export function definePlugin(definition: PluginDefinition): SprintPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: SprintPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@sprintai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@sprintai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@sprintai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@sprintai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@sprintai/plugin-sdk/ui`
...
The `@sprintai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Sprint secret provider system rather than a plain text input
...
1. Core Sprint data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Sprint objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Sprint product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Sprint later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Sprint secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm sprintai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Sprint server. This is a normative requirement, not optional.
...
### 27.1 `@sprintai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@sprintai/plugin-test-harness";
...
For developing a plugin against a running Sprint instance:

- The operator installs the plugin from a local path: `pnpm sprintai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-sprint-plugin`) that scaffolds:

- `package.json` with correct `sprintPlugin` keys
...
- `@sprint/plugin-workspace-files`
- `@sprint/plugin-terminal`
- `@sprint/plugin-git`
- `@sprint/plugin-linear`
- `@sprint/plugin-github-issues`
- `@sprint/plugin-grafana`
- `@sprint/plugin-runtime-processes`
- `@sprint/plugin-stripe`
...
- `@sprintai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@sprintai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@sprintai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@sprintai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@sprintai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@sprintai/plugin-sdk/ui`
...
- `@sprintai/plugin-test-harness`
- `create-sprint-plugin` starter template
...
Sprint should not implement a generic in-process hook bag modeled directly after local coding tools.

Sprint should implement:
...
That is the complete target design for the Sprint plugin system.
