...
ClipHub is the public registry where people share, discover, and download Sprint company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Sprint what a package registry is to a programming language. Sprint already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Sprint — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Sprint instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Sprint spec. It contains:
...
**Install (fresh start):**
```
sprint install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Sprint instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Sprint, export your company as a template, then publish:

```
sprint export --template my-company
sprint publish cliphub my-company
```
...
ClipHub is a **separate service** from Sprint itself. Sprint is self-hosted; ClipHub is a hosted registry that Sprint instances talk to.
...
| **Sprint CLI** | `sprintai install`, `sprintai publish`, `sprintai cliphub sync` — built into Sprint |
| **Sprint UI** | "Browse ClipHub" panel in the Sprint web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Sprint) |
| Backend | TypeScript + Hono (consistent with Sprint) |
| Database | PostgreSQL |
...
4. Run `sprintai install cliphub:acme/lean-saas-shop`
5. Sprint creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Sprint until it works well
2. Export: `sprintai export --template my-agency`
3. Publish: `sprintai publish cliphub my-agency`
...
3. Install just that agent: `sprintai install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use Sprint. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Sprint what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `sprintai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Sprint web UI
- [ ] `sprintai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Sprint's own philosophy)
