...
This spec defines how Sprint actually runs agents while staying runtime-agnostic.
...
1. Sprint is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Sprint manages lifecycle, and on exit Sprint parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `sprint.skill` (shared Sprint skill text block)
...
2. Preferred transport is env vars (`SPRINT_*`) injected at runtime.
