...
Audience: Operators setting up and running agents in Sprint

## 1. What this system does

Agents in Sprint do not run continuously.  
...
For `claude_local` and `codex_local`, Sprint assumes the CLI is already installed and authenticated on the host machine.
...
Sprint stores session IDs for resumable adapters.
...
Sprint pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Sprint surfaces this as a warning in environment tests, not a hard error.
