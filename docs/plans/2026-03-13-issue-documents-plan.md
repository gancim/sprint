...
Add first-class **documents** to Sprint as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Sprint workflow/docs.
...
Update the Sprint agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/sprint/SKILL.md`
