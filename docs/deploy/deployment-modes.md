...
Sprint supports two runtime modes with different security profiles.
...
```sh
pnpm sprintai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Sprint emits a one-time claim URL at startup:
...
```sh
SPRINT_DEPLOYMENT_MODE=authenticated pnpm sprintai run
```
