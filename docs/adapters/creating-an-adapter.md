...
Build a custom adapter to connect Sprint to any agent runtime.
...
2. Build environment with `buildSprintEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `sprintai run --watch` using `picocolors`.
...
Make Sprint skills discoverable to your agent runtime without writing to the agent's working directory:
