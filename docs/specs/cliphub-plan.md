# ClipHub: Marketplace for Sprint Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Sprint configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Sprint-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Sprint company (agents, projects, governance) |
...
A complete Sprint company configuration:
...
Single-agent configurations designed to plug into a Sprint org:
...
Portable skill files that any Sprint agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Sprint's skill loading system
...
interface Listing {
  ...
  sprintVersionMin: string;     // Minimum Sprint version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Sprint company
...
| `POST` | `/api/listings/:id/install` | Install to Sprint company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Sprint company (or create new)
  → ClipHub API calls Sprint API to:
  ...
  → Redirect to Sprint dashboard with new team running
```
...
### 5.3 Creator: Export from Sprint → Publish

```
Running Sprint company → "Export as Blueprint" (CLI or UI)
  → Sprint exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Sprint brand blue/purple)
...
| Install flow | One-click deploy to Sprint company |
...
{
  "targetCompanyId": "uuid",         // Existing Sprint company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `sprint-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Sprint
- **Backend**: Node.js API (or extend Sprint server)
- **Database**: Postgres (can share Sprint's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Sprint auth (or OAuth2)

### 9.2 Integration with Sprint

ClipHub can be:
- **Option A**: A separate app that calls Sprint's API to install blueprints
- **Option B**: A built-in section of the Sprint UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Sprint UI and API.
...
- [ ] Install flow: blueprint → Sprint company
...
- [ ] "Export from Sprint" CLI command
