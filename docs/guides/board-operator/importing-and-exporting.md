...
Sprint companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .sprint.yaml     # Adapter config, env inputs, routines
```
...
- **.sprint.yaml** holds Sprint-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
sprintai company export <company-id> --out ./my-export
```
...
sprintai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
sprintai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
sprintai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.sprint.yaml`
...
```sh
# From a local folder
sprintai company import ./my-export

# From a GitHub URL
sprintai company import https://github.com/org/repo

# From a GitHub subfolder
sprintai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
sprintai company import org/repo
sprintai company import org/repo/companies/acme
```
...
If `--target` is not specified, Sprint infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
sprintai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
sprintai company import org/company-templates/engineering-team \
  --target new \
...
```sh
sprintai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
sprintai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
sprintai company import ./package \
  --target new \
  --yes \
  --json
```
...
Sprint supports several GitHub URL formats:
