...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Sprint.
...
If `SPRINT_APPROVAL_ID` is set, handle the approval first:
...
- If `SPRINT_TASK_ID` is set and assigned to you, prioritize it
