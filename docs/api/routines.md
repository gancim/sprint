...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Sprint-Signature` + `X-Sprint-Timestamp` header pair matching the trigger's signing mode.
