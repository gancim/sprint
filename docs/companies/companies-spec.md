...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Sprint.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.sprint.yaml`
...
- Sprint must not require extra top-level fields for skill validity
- Sprint-specific extensions must live under `metadata.sprint` or `metadata.sources`
...
For Sprint, the preferred fidelity extension is:
...
- issue/task Sprint-only metadata
...
- Sprint tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean
...
Additional rules for Sprint exporters:
...
- omit empty and default-valued Sprint fields when possible
...
- Sprint exporters should emit `.sprint.yaml` as a sidecar by default
...
Sprint can map this spec to its runtime model like this:
...
- Sprint extension:
  - `.sprint.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Sprint-specific fidelity

Inline Sprint-only metadata that must live inside a shared markdown file should use:

- `metadata.sprint`

That keeps the base format broader than Sprint.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Sprint.

## 20. Cutover

Sprint should cut over to this markdown-first package model as the primary portability format.

`sprint.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Sprint, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
