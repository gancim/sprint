#!/usr/bin/env bash
#
# Kill all local Sprint dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all sprint dev processes
...
# Collect PIDs of node processes running from any sprint directory.
# Matches paths like /Users/*/sprint/... or /Users/*/sprint-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/sprint(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No Sprint dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} Sprint dev process(es):"
echo ""
