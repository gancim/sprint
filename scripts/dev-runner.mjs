...
const devServerStatusFilePath = path.join(repoRoot, ".sprint", "dev-server-status.json");
...
const env = {
  ...process.env,
  SPRINT_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.SPRINT_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.SPRINT_MIGRATION_PROMPT ??= "never";
  env.SPRINT_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.SPRINT_DEPLOYMENT_MODE = "authenticated";
  env.SPRINT_DEPLOYMENT_EXPOSURE = "private";
  env.SPRINT_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[sprint] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[sprint] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@sprintai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[sprint] Command failed with code ${status.code}: pnpm --filter @sprintai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[sprint] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.SPRINT_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[sprint] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[sprint] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@sprintai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[sprint] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@sprintai/server", serverScript, ...forwardedArgs],
