...
SPRINT_HOST_PORT="${SPRINT_HOST_PORT:-3100}"
SPRINT_HOST_FROM_CONTAINER="${SPRINT_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.sprint-openclaw.override.yml"
...
detect_sprint_base_url() {
  ...
  for candidate in "$SPRINT_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${SPRINT_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${SPRINT_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
sprint_base_url="$(detect_sprint_base_url || true)"
...
Sprint URL for OpenClaw container:
EOF
  if [[ -n "$sprint_base_url" ]]; then
    cat <<EOF
  $sprint_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${SPRINT_HOST_PORT}
  (Do not use http://127.0.0.1:${SPRINT_HOST_PORT} inside the container.)
  If Sprint rejects the host, run on host machine:
    pnpm sprintai allowed-hostname host.docker.internal
  Then restart Sprint and re-run this script.
EOF
  fi
  cat <<EOF
