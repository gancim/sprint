...
COMPANY_ID="${COMPANY_ID:-${SPRINT_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-sprint-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-sprint-openclaw-smoke}"
...
Provide one of:
  SPRINT_AUTH_HEADER=\"Bearer <board-token>\"
  SPRINT_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Sprint OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.sprint.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
