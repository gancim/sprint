...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Sprint data dir to use
  SPRINT_HOST=127.0.0.1    Host passed to the onboarded server
  SPRINT_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/sprint-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/sprint-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export SPRINT_HOST="${SPRINT_HOST:-127.0.0.1}"
export SPRINT_PORT="${SPRINT_PORT:-3100}"
export SPRINT_OPEN_ON_LISTEN="${SPRINT_OPEN_ON_LISTEN:-false}"
...
echo "SPRINT_HOST: $SPRINT_HOST"
echo "SPRINT_PORT: $SPRINT_PORT"
...
env \
  ...
  HOST="$SPRINT_HOST" \
  PORT="$SPRINT_PORT" \
  SPRINT_OPEN_ON_LISTEN="$SPRINT_OPEN_ON_LISTEN" \
  pnpm sprintai onboard --yes --data-dir "$PC_DATA"
