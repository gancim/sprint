...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@sprintai/db";
import type { DeploymentMode } from "@sprintai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  sprintUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).sprintUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.sprintUpgradeContext = context;
