...
function jwtConfig() {
  const secret = process.env.SPRINT_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.SPRINT_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.SPRINT_AGENT_JWT_ISSUER ?? "sprint",
    audience: process.env.SPRINT_AGENT_JWT_AUDIENCE ?? "sprint-api",
  };
}
