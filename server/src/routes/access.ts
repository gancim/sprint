...
import type { DeploymentExposure, DeploymentMode } from "@sprintai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "sprint" &&
    normalized !== "sprint-create-agent" &&
    normalized !== "sprint-create-plugin" &&
...
/** Resolve the Sprint repo skills directory (built-in / managed skills). */
function resolveSprintSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const sprintSkillsDir = resolveSprintSkillsDir();

  // Build set of Sprint-managed skill names
  const sprintSkillNames = new Set<string>();
  if (sprintSkillsDir) {
    try {
      for (const entry of fs.readdirSync(sprintSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) sprintSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isSprintManaged: sprintSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.sprintApiUrl)) {
    const legacySprintApiUrl = nonEmptyTrimmedString(input.sprintApiUrl);
    if (legacySprintApiUrl) merged.sprintApiUrl = legacySprintApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    sprintApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.sprintApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawSprintApiUrl =
    typeof defaults.sprintApiUrl === "string"
      ? defaults.sprintApiUrl.trim()
      : "";
  if (rawSprintApiUrl) {
    try {
      const parsedSprintApiUrl = new URL(rawSprintApiUrl);
      if (
        parsedSprintApiUrl.protocol !== "http:" &&
        parsedSprintApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_sprint_api_url_protocol",
          level: "warn",
          message: `sprintApiUrl must use http:// or https:// (got ${parsedSprintApiUrl.protocol}).`
        });
      } else {
        normalized.sprintApiUrl = parsedSprintApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_sprint_api_url_configured",
          level: "info",
          message: `sprintApiUrl set to ${parsedSprintApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_sprint_api_url_invalid",
        level: "warn",
        message: `Invalid sprintApiUrl: ${rawSprintApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Sprint host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Sprint is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm sprintai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/sprint";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/sprint-claimed-api-key.json and load SPRINT_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: sprintApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Sprint hostname is reachable and allowed via `pnpm sprintai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Sprint API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/sprint/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Sprint OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          sprintApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Sprint generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "sprintApiUrl": "https://sprint-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Sprint before key claim is allowed.
    ...
    - ~/.openclaw/workspace/sprint-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/sprint-claimed-api-key.json

    And set the SPRINT_API_KEY and SPRINT_API_URL in your environment variables as specified here:
    ...
    Then set SPRINT_API_KEY and SPRINT_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Sprint skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your SPRINT_API_URL to the top of your skill and note the path to your SPRINT_API_URL
    ...
    ${
      ...
      "Ensure Sprint is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Sprint base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.sprintApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm sprintai allowed-hostname <host>
      - then restart Sprint and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@sprint.local";
...
export function resolveJoinRequestAgentManagerId(
  ...
  const ceoCandidates = candidates.filter(
    (candidate) => candidate.role === "scrum_master"
  );
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "sprint", path: "/api/skills/sprint" },
        {
        ...
        },
        {
          ...
          path: "/api/skills/sprint-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              sprintApiUrl: req.body.sprintApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.sprintApiUrl &&
          !persistedDefaults.sprintApiUrl
        ) {
          missingPersistedFields.push("sprintApiUrl");
        }
