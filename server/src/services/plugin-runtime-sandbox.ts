...
import type { SprintPluginManifestV1 } from "@sprintai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __sprint_exports: module.exports,
      __sprint_module: module,
      __sprint_require: requireInSandbox,
      __sprint_filename: realPath,
      __sprint_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__sprint_exports, __sprint_module, __sprint_require, __sprint_filename, __sprint_dirname)`;
