...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".sprint.yaml" || normalized === ".sprint.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const sprint = isPlainRecord(metadata?.sprint) ? metadata?.sprint as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.sprintSkillKey)
    ?? asString(sprint?.skillKey)
    ?? asString(sprint?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "sprint_bundled") {
    return `sprintai/sprint/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .sprint.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .sprint.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .sprint.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .sprint.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Sprint will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .sprint.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .sprint.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[sprintExtensionPath];
  if (selectedFiles.has(sprintExtensionPath) && typeof extensionEntry === "string") {
    filtered[sprintExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findSprintExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".sprint.yaml"] === "string") return ".sprint.yaml";
  if (typeof files[".sprint.yml"] === "string") return ".sprint.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.sprint.yaml") || entry.endsWith("/.sprint.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "sprint_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "sprintai/sprint",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/sprintai/sprint/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "sprint_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.sprintSkillKey = skill.key;
  metadata.sprint = {
    ...(isPlainRecord(metadata.sprint) ? metadata.sprint : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const sprintExtensionPath = findSprintExtensionPath(normalizedFiles);
  const sprintExtension = sprintExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, sprintExtensionPath) ?? "")
    : {};
  const sprintCompany = isPlainRecord(sprintExtension.company) ? sprintExtension.company : {};
  const sprintSidebar = normalizePortableSidebarOrder(sprintExtension.sidebar);
  const sprintAgents = isPlainRecord(sprintExtension.agents) ? sprintExtension.agents : {};
  const sprintProjects = isPlainRecord(sprintExtension.projects) ? sprintExtension.projects : {};
  const sprintTasks = isPlainRecord(sprintExtension.tasks) ? sprintExtension.tasks : {};
  const sprintRoutines = isPlainRecord(sprintExtension.routines) ? sprintExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      logoPath: asString(sprintCompany.logoPath) ?? asString(sprintCompany.logo),
    },
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(sprintAgents[slug]) ? sprintAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(sprintProjects[slug]) ? sprintProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(sprintTasks[slug]) ? sprintTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(sprintRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(sprintRoutines[slug]) ? sprintRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const sprintAgentsOut: Record<string, Record<string, unknown>> = {};
    const sprintProjectsOut: Record<string, Record<string, unknown>> = {};
    const sprintTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const sprintRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readSprintSkillSyncPreference(
        ...
        sprintAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      sprintProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      sprintTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      sprintRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const sprintExtensionPath = ".sprint.yaml";
    const sprintAgents = Object.fromEntries(
      Object.entries(sprintAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const sprintProjects = Object.fromEntries(
      Object.entries(sprintProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const sprintTasks = Object.fromEntries(
      Object.entries(sprintTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const sprintRoutines = Object.fromEntries(
      Object.entries(sprintRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[sprintExtensionPath] = buildYamlFile(
      {
        schema: "sprint/v1",
        ...
        agents: Object.keys(sprintAgents).length > 0 ? sprintAgents : undefined,
        projects: Object.keys(sprintProjects).length > 0 ? sprintProjects : undefined,
        tasks: Object.keys(sprintTasks).length > 0 ? sprintTasks : undefined,
        routines: Object.keys(sprintRoutines).length > 0 ? sprintRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, sprintExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeSprintSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
