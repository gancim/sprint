...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@sprintai/db";
 ...
 *
 * This implementation delegates to the core Sprint domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@sprintai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
