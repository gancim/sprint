 ...
 *    (`~/.sprint/plugins/`) and `node_modules` for packages matching
 *    the `sprint-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Sprint plugins.
 * Packages matching this pattern are considered Sprint plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "sprint-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: SprintPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.sprint/plugins/ local directory
  | "npm"               // npm packages matching sprint-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: SprintPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the sprint-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "sprint-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: SprintPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Sprint plugins installed as npm packages in the current
   * Node.js environment matching the "sprint-plugin-*" naming convention.
   *
   ...
   * the "sprintPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Sprint plugin.
   * Throws if the package is a Sprint plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<SprintPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: SprintPluginManifestV1;
    newManifest: SprintPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Sprint plugin naming convention.
 * Accepts both the "sprint-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @sprintai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "sprintPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const sprintPlugin = pkgJson["sprintPlugin"];
  if (
    sprintPlugin !== null &&
    typeof sprintPlugin === "object" &&
    !Array.isArray(sprintPlugin)
  ) {
    const manifestRelPath = (sprintPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: SprintPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumSprintVersion;
}
 ...
 *   packageName: "sprint-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: SprintPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as SprintPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Sprint plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<SprintPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Sprint plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasSprintPlugin = "sprintPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasSprintPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<SprintPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: SprintPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @sprintai/shared exports). Run those workers through
