...
import { companySkills } from "@sprintai/db";
import { readSprintSkillSyncPreference, writeSprintSkillSyncPreference } from "@sprintai/adapter-utils/server-utils";
import type { SprintSkillEntry } from "@sprintai/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@sprintai/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveSprintInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("sprintai/sprint/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveSprintInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "sprint_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Sprint skills are read-only.",
      sourceLabel: "Sprint bundled",
      sourceBadge: "sprint",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "sprint_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Sprint skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "sprint_bundled"
        && incomingKind === "github"
        && incomingOwner === "sprintai"
        && incomingRepo === "sprint"
      ) {
        out.push(existing);
        continue;
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeSprintSkillSyncPreference(config, filtered),
        });
      }
    }
