 ...
 */
import { pluginManifestV1Schema } from "@sprintai/shared";
import type { SprintPluginManifestV1 } from "@sprintai/shared";
import { PLUGIN_API_VERSION } from "@sprintai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): SprintPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as SprintPluginManifestV1,
        };
      }
