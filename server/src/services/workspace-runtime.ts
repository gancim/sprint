...
import type { AdapterRuntimeServiceReport } from "@sprintai/adapter-utils";
import type { Db } from "@sprintai/db";
import { workspaceRuntimeServices } from "@sprintai/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.SPRINT_WORKSPACE_CWD = input.worktreePath;
  env.SPRINT_WORKSPACE_PATH = input.worktreePath;
  env.SPRINT_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.SPRINT_WORKSPACE_BRANCH = input.branchName;
  env.SPRINT_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.SPRINT_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.SPRINT_WORKSPACE_SOURCE = input.base.source;
  env.SPRINT_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.SPRINT_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.SPRINT_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.SPRINT_PROJECT_ID = input.base.projectId ?? "";
  env.SPRINT_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.SPRINT_AGENT_ID = input.agent.id;
  env.SPRINT_AGENT_NAME = input.agent.name;
  env.SPRINT_COMPANY_ID = input.agent.companyId;
  env.SPRINT_ISSUE_ID = input.issue?.id ?? "";
  env.SPRINT_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.SPRINT_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.SPRINT_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.SPRINT_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.SPRINT_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.SPRINT_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.SPRINT_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.SPRINT_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.SPRINT_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.SPRINT_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.SPRINT_PROJECT_ID = input.workspace.projectId ?? "";
  env.SPRINT_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.SPRINT_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".sprint", "worktrees");
