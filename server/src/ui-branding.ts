const FAVICON_BLOCK_START = "<!-- SPRINT_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- SPRINT_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- SPRINT_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- SPRINT_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.SPRINT_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.SPRINT_WORKTREE_NAME) ?? nonEmpty(env.SPRINT_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.SPRINT_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="sprint-worktree-enabled" content="true" />',
    `<meta name="sprint-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="sprint-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="sprint-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
