...
import type { WorkspaceOperation } from "@sprintai/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sprint-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "sprint@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Sprint Test"]);
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.branchName).toBe("SPR-447-add-worktree-support");
    expect(first.cwd).toContain(path.join(".sprint", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$SPRINT_WORKSPACE_BRANCH\" > .sprint-provision-branch",
        "printf '%s\\n' \"$SPRINT_WORKSPACE_BASE_CWD\" > .sprint-provision-base",
        "printf '%s\\n' \"$SPRINT_WORKSPACE_CREATED\" > .sprint-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".sprint-provision-branch"), "utf8")).resolves.toBe(
      "SPR-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".sprint-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".sprint-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".sprint-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("records worktree setup and provision operations when a recorder is provided", async () => {
    ...
    expect(operations[0]?.metadata).toMatchObject({
      branchName: "SPR-540-record-workspace-operations",
      created: true,
    });
  ...
  it("reuses an existing branch without resetting it when recreating a missing worktree", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "SPR-450-recreate-missing-worktree";
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sprint-runtime-workspace-"));
  ...
  it("does not leak parent Sprint instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sprint-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "sprintConfig: process.env.SPRINT_CONFIG ?? null,",
          "sprintHome: process.env.SPRINT_HOME ?? null,",
          "sprintInstanceId: process.env.SPRINT_INSTANCE_ID ?? null,",
    ...
    process.env.SPRINT_CONFIG = "/tmp/base-sprint-config.json";
    process.env.SPRINT_HOME = "/tmp/base-sprint-home";
    process.env.SPRINT_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/sprint";
    ...
    expect(captured.sprintConfig).toBeNull();
    expect(captured.sprintHome).toBeNull();
    expect(captured.sprintInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sprint-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "sprint-runtime-sibling-"));
