...
describe("agent local JWT", () => {
  const secretEnv = "SPRINT_AGENT_JWT_SECRET";
  const ttlEnv = "SPRINT_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "SPRINT_AGENT_JWT_ISSUER";
  const audienceEnv = "SPRINT_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "sprint";
    process.env[audienceEnv] = "sprint-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
