...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "sprint", LOGNAME: "sprint", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "sprint" }),
      },
    );

    expect(tokens).toEqual(["sprint", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "sprint" }, {
        userInfo: () => ({ username: "sprint" }),
      });

      expect(tokens).toEqual(["sprint", "custom-token"]);
    } finally {
