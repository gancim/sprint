...
describe("agent instructions service", () => {
  const originalSprintHome = process.env.SPRINT_HOME;
  const originalSprintInstanceId = process.env.SPRINT_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalSprintHome === undefined) delete process.env.SPRINT_HOME;
    else process.env.SPRINT_HOME = originalSprintHome;
    if (originalSprintInstanceId === undefined) delete process.env.SPRINT_INSTANCE_ID;
    else process.env.SPRINT_INSTANCE_ID = originalSprintInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const sprintHome = await makeTempDir("sprint-agent-instructions-home-");
    const externalRoot = await makeTempDir("sprint-agent-instructions-external-");
    cleanupDirs.add(sprintHome);
    cleanupDirs.add(externalRoot);
    process.env.SPRINT_HOME = sprintHome;
    process.env.SPRINT_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("sprint-agent-instructions-new-external-");
    cleanupDirs.add(sprintHome);
    cleanupDirs.add(externalRoot);
    process.env.SPRINT_HOME = sprintHome;
    process.env.SPRINT_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("sprint-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const sprintHome = await makeTempDir("sprint-agent-instructions-recover-");
    cleanupDirs.add(sprintHome);
    process.env.SPRINT_HOME = sprintHome;
    process.env.SPRINT_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const sprintHome = await makeTempDir("sprint-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("sprint-agent-instructions-stale-root-");
    cleanupDirs.add(sprintHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const sprintHome = await makeTempDir("sprint-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("sprint-agent-instructions-heal-write-stale-");
    cleanupDirs.add(sprintHome);
    cleanupDirs.add(staleRoot);
    process.env.SPRINT_HOME = sprintHome;
    process.env.SPRINT_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const sprintHome = await makeTempDir("sprint-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("sprint-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(sprintHome);
    cleanupDirs.add(staleRoot);
    process.env.SPRINT_HOME = sprintHome;
    process.env.SPRINT_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const sprintHome = await makeTempDir("sprint-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("sprint-agent-instructions-partial-root-");
    cleanupDirs.add(sprintHome);
    cleanupDirs.add(staleRoot);
    process.env.SPRINT_HOME = sprintHome;
    process.env.SPRINT_INSTANCE_ID = "test-instance";
