...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Sprint OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Sprint base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("sprintApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.sprintApiUrl");
    expect(text).toContain("~/.openclaw/workspace/sprint-claimed-api-key.json");
    expect(text).toContain("SPRINT_API_KEY");
