...
<head>
    <!-- SPRINT_RUNTIME_BRANDING_START -->
    <!-- SPRINT_RUNTIME_BRANDING_END -->
    <!-- SPRINT_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from SPRINT_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ SPRINT_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SPRINT_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SPRINT_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      SPRINT_IN_WORKTREE: "true",
      SPRINT_WORKTREE_NAME: "sprint-pr-432",
      SPRINT_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("sprint-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="sprint-worktree-name"');
    expect(meta).toContain('content="sprint-pr-432"');
    expect(meta).toContain('name="sprint-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="sprint-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="sprint-worktree-name"');
  });
});
