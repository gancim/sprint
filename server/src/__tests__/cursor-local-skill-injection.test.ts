...
import { ensureCursorSkillsInjected } from "@sprintai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Sprint skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("sprint-cursor-skills-src-");
    const skillsHome = await makeTempDir("sprint-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "sprint");
    await createSkillDir(skillsDir, "sprint-create-agent");
    ...
    const injectedA = path.join(skillsHome, "sprint");
    const injectedB = path.join(skillsHome, "sprint-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "sprint")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "sprint-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "sprint"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "sprint-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("sprint-cursor-preserve-src-");
    const skillsHome = await makeTempDir("sprint-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "sprint");
    ...
    expect((await fs.lstat(path.join(skillsHome, "sprint-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("sprint-cursor-fail-src-");
    const skillsHome = await makeTempDir("sprint-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
